...
void MD5_Update(MD5_CTX *ctx, const void *data, unsigned long size)
{
    ...
    if (used) {
        ...
        if (size < available) {
            for (int i = 0; i < size; i++) {
                ctx->buffer[used+i] = *((unsigned char *)data + i);
            }
            return;
        }
    ...
    for (int i = 0; i < size; i++) {
        ctx->buffer[i] = *((unsigned char *)data + i);
    }
}
...
__kernel void crackMD5(__global const unsigned char *hash,
                       __global const unsigned char *keySpace,
                       __global char *result,
                       __global char *passwordLen,
                       __global volatile int* flag)
{
    __private int id[2] = { get_global_id(1), get_global_id(2) };

    __private char string[MAX_STRING_SIZE + 1] = {
        keySpace[get_global_id(0)],
        0x61 + (id[0]/NUM_CHARS),
        0x61 + (id[0]%NUM_CHARS),
        0x61 + (id[1]/NUM_CHARS),
        0x61 + (id[1]%NUM_CHARS),
    ...
    __private unsigned char iters = 1;

    for(int len = 1; len <= MAX_STRING_SIZE; len++) {
        ...
        // Calculate work size
        for (__private unsigned char j = 0; j < iters; j++)
        {
            string[5] = 0x61 + j;

            if(atomic_add(flag, 0)) {
                return;
            }
